...
import com.mt.mtengine.entity.room.BickerMatch
...
interface BickerRoomDao : BaseRoomDao<BickerMatch, String> {
    ...
    override fun findByCompanyIdAll(companyId: Iterable<Int>): Flux<BickerMatch>

    @Query("select room_id, time, enable, start_time from mt_room_bicker")
    override fun findTimeAll(): Flux<BickerMatch>

    @Query("select company_id, stock_id from mt_room_bicker where room_id = :roomId")
    override fun findBaseByRoomId(roomId: String): Mono<BickerMatch>
}