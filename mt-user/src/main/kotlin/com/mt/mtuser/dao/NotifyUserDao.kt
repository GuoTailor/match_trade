...
interface NotifyUserDao : CoroutineCrudRepository<NotifyUser, Int> {
    ...
    @Query("select count(1) from mt_notify_user where user_id = :userId and status = '${NotifyUser.unread}' and msg_type = :msgType")
    ...
    @Query("select msg_id from mt_notify_user where user_id = :userId and status = 'unread' and msg_type = '${NotifyUser.typeMsg}'")
    ...
    @Modifying
    @Query("update mt_notify_user set status = :status where user_id = :userId and msg_id = :msgId and msg_type = :msgType")
    suspend fun setStatusByUserIdAndMsgId(userId: Int, msgId: Int, status: String, msgType: String): Int
}