...
interface TradeInfoDao : CoroutineCrudRepository<TradeInfo, Int> {
    ...
    @Query("select COALESCE(sum(trade_amount), 0) from mt_trade_info where trade_time > :time")
    suspend fun countStockByTradeTime(time: Date): Long

    @Query("select COALESCE(sum(trade_amount), 0) from mt_trade_info where trade_time > :time and company_id = :companyId")
    suspend fun countStockByTradeTimeAndCompanyId(time: Date, companyId: Int): Long

    @Query("select COALESCE(sum(trade_amount), 0) from mt_trade_info where trade_time > :time and stock_id = :stockId")
    suspend fun countStockByTradeTimeAndStockId(time: Date, stockId: Int): Long

    @Query("select COALESCE(sum(trade_money), 0) from mt_trade_info where trade_time > :time")
    suspend fun countMoneyTradeTime(time: Date): Long

    @Query("select COALESCE(sum(trade_money), 0) from mt_trade_info where trade_time > :time and company_id = :companyId")
    suspend fun countMoneyByTradeTimeAndCompanyId(time: Date, companyId: Int): Long

    @Query("select COALESCE(sum(trade_money), 0) from mt_trade_info where trade_time > :time and stock_id = :stockId")
    suspend fun countMoneyByTradeTimeAndStockId(time: Date, stockId: Int): Long

    @Query("select COALESCE(max(trade_price), 0) from mt_trade_info " +
    ...
    @Query("select COALESCE(avg(trade_price), 0) from mt_trade_info " +
            " where trade_time >= :startTime " +
            " and trade_time <= :endTime " +
            " and company_id = :companyId ")
