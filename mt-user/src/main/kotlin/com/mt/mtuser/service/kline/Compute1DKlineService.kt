...
class Compute1DKlineService : ComputeKline() {
    ...
    override suspend fun isExist(time: LocalDateTime, stockId: Int, companyId: Int): Boolean {
        return connect.execute("select * from $tableName where time = :time and stock_id = :stockId and company_id = :companyId limit 1")
                .bind("time", time)
                .bind("stockId", stockId)
                .bind("companyId", companyId)
                .`as`(Kline::class.java)
                .fetch().awaitOneOrNull() != null
    }
    ...
    suspend fun computeCurrent(stockId: Int, companyId: Int) {
        ...
        val newKline = connect.execute("select count(1) as tradesNumber," +
