 ...
 */
@Service
class Compute4HKlineService : ComputeKline() {
    override val tableName: String = "mt_4h_kline"
    override fun handlerRequest(time: LocalDateTime): Boolean {
        return time.minute == 0 && time.hour % 4 == 0 && time.plusSeconds(stepOfSeconds).isBefore(LocalDateTime.now())
    }

    val stepOfSeconds = 4 * 60 * 60L

    @Autowired
    private lateinit var template: R2dbcEntityTemplate

    override fun formatDate(time: LocalDateTime): LocalDateTime {
        return time.withHour(time.hour / 4 * 4)
            .withMinute(0)
            .withSecond(0)
            .withNano(0)
    }
    ...
    override suspend fun getMinComputeTime(): LocalDateTime? {
        return template.databaseClient.sql("select min(time) as time from mt_1h_kline")
    ...
    override suspend fun compute(stockId: Int, companyId: Int, time: LocalDateTime, offset: Long): Kline {
        ...
        kline.time = endTime
        kline.openPrice = klineService.getOpenPriceByTableName(startTime, endTime, stockId, "mt_1h_kline")
